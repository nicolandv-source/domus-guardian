...
class CoreIdentityAdapter:
    ...
    async def create_identity(self) -> str:
        """Mint a new provisional DAID. Raises on any non-2xx or network
        failure; the caller decides how that counts against ``attempts``."""
        async with httpx.AsyncClient(timeout=self._timeout_seconds) as client:
            response = await client.post(f"{self._base_url}/core/v1/identities")
